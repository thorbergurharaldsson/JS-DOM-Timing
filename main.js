...
modifyTable = () => {
  //uses setTimeout to display a loading animation before modifying the table
  loadingAnimation(open);
  setTimeout(() => loadingAnimation(close), 500);
...
// used to show/hide loading animation
loadingAnimation = (action) => {
  const loading = document.querySelector(".loading"); //Finds an element with the class .loading

  // checks what is parsed to the function
  switch (action) {
    case open: //if it's loadingAnimation(open)
      loading.classList.add("is-loading"); // then add the css class .is-loading to the element, makes the opacity: 1; see line 95 in style.css
      break;
    case close: //if it's loadingAnimation(close)
      loading.classList.remove("is-loading"); // then remove the css class .is-loading to the element, makes the opacity: 0; see line 90 in style.css
      break;
    // default action is to replace, what ever classes are on the element with loading, essentially resetting it
    default:
      loading.classList.replace("loading");
      break;
  }
};
